          ...
          <div className={cn('mx-auto mt-2 max-w-6xl px-6 transition-all duration-300 lg:px-12', isScrolled && 'bg-background/50 max-w-4xl rounded-2xl border backdrop-blur-lg lg:px-5')}>
            <div className="relative flex flex-wrap items-center justify-between gap-6 py-2 lg:gap-0 lg:py-3">
              ...
              <div className="absolute inset-0 m-auto hidden size-fit lg:block">
                <ul className="flex border border-primary text-primary font-semibold">
                  {menuItems.map((item, index) => (
                    <li key={index}>
                        <Link href={item.href}>
                          <button className="px-5 py-1 hover:bg-primary/90 hover:text-white cursor-pointer">{item.name}</button>
                        </Link>
                    </li>
                  ))}
                </ul>
